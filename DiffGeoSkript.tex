...
\begin{definition}
	...
	Die \textit{Tangente} an einem Punkt $X(t)$, wird durch den Punkt und seinen \textit{Tangentialvektor} $X'(t)$ beschrieben. $\mathcal{T}(t)=X(t) + [X'(t)]$ notiert diese Gerade.
...
\begin{definition}
	...
	\begin{enumerate}
		\item \textit{Tangentialfeld} entlang $X$, falls \[ \forall t \in I: Y(t) \in T_tX \]
		\item \textit{Normalenfeld} entlang $X$, falls \[  \forall t \in I: Y(t) \in N_tX \]
	\end{enumerate}
\end{definition}

\begin{remark, definition}

Jede Kurve hat ein \textbf{(und nur ein!)} harmonisches \textit{Einheitstangentenfeld} (ETF)
...
\begin{lemma, definition}
	...
	Der \textit{(angepasste) Rahmen} eines Streifens $(X,N): I\rightarrow\E^3\times S^2$ ist eine Abbildung \[ F=(T,N,B): I\rightarrow SO(V) \] seine \textit{Strukturgleichungen} sind von der Form \[ F' = F \phi \text{ mit } \phi =|X'| \begin{pmatrix}
	0 & - \kappa_n & \kappa_g \\
	\kappa_n & 0 & -\tau\\
	-\kappa_g & \tau & 0
...
\begin{remark}
	...
	\begin{enumerate}
		...
		\item ist $s \mapsto (\widetilde{X}, \widetilde{N})(s) = (X,N)(t(s))$ eine \textit{orientierungstreue Umparametrisierung}, d.h., $t' >0$, von $t \mapsto (X,N)(t)$, so gilt
...
\begin{definition}
	$X: I \rightarrow \E^3$ heißt \textit{Frenet Kurve}, falls \[ \forall t \in I: (X' \times X'')(t) \not = 0. \]
\end{definition}
...
\begin{lemma, definition}
	Ist $X: I \rightarrow \E^3$ Frenet mit Hauptnormale $N: I \rightarrow V$, so sind die Strukturgleichungen des \textit{Frenet Rahmens} $F = (T,N,B)$ der Kurve die \textit{Frenet-Serret Gleichungen} $F' = F\phi$ mit $\mathcal{C} = |X'|\begin{pmatrix}
	0 & - \kappa & 0\\
	\kappa & 0 & - \tau \\
	0 & \tau & 0
	\end{pmatrix}$
	 mit der \textit{Krümmung} $\kappa > 0$ und \textit{Torsion} $\tau$ der Frenet Kurve $X$.
\end{lemma, definition}
