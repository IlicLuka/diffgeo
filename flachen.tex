...
\begin{definition}
	...
	Das \emph{Tangential-} bzw. \emph{Normalenbündel} einer Fläche $ X: M \rightarrow \E $ ist gegeben durch die Abbildung 
		\[ (u,v) \mapsto T_{(u,v)}X := [ \{X_u,X_v\} ](u,v), \]
		\[ (u,v) \mapsto N_{(u,v)}X := \{ X_u,X_v \}^{\perp}(u,v). \]
...
\begin{example}
	...
	jede Profilkurve $ v \equiv \mathrm{const} $ ist der orthogonale Schnitt der Fläche mit der Ebene $ x\sin(v) = y \cos(v) $ der Meridiankurve; die Gaußabbildung erhält man also durch $ \dfrac{\pi}{2} $ Drehung des ETF (''Einheitstangentialfel'') in der Ebene der Kurve 
...
\begin{remark}
	...
	Die Gaußabbildund einer Fläche ist ein geometrisches Objekt:
	Nach einer Euklid. Bewegung
	...
	Eine Spiegelung liefert $ \widetilde{N} = -AN, $ d.h., wechselt das Vorzeichen -- was auch einer (ordnungsumkehrende) Umparametrisierung tut, z.B.: $ (u,v) \mapsto(v,u) $.
	Demnach; $ N $ ist ''geometrisch'' bis auf Vorzeichen.
...
\begin{lemma, definition}
	...
	Die Ableitung der Gaußabbildung ist tangential-wertig,
	...
	\[ \s\big|_{(u,v)}:= - d_{(u,v)N}= (d_{(u,v)X})^{-1} \in \mathrm{End(T_{(u,v)}X)}. \]
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Die Abbildung von N ist tangentialwertig. Nämlich:
...
\begin{remark}
	Die Abbildung $\mapsto \s \big|_{(u,v)} \in End(T(u,v),X)$ liefert ein Endomorphismenfeld $\s$, welches man auch \emph{Weingartentensorfeld} nennt.
\end{remark}

\begin{remark}
	Da $\left( X_u(u,v) , X_v(u,v) \right)$ eine Basis von $T(u,v)X$ ist, kann $\s \big|_{(u,v)}$ durch die Werte auf der Basis bestimmt werden
...
\begin{lemma}
	$\s \big|_{(u,v)} \in End(T(u,v),X)$ ist symmetrisch für jedes $(u,v) \in M$.
\end{lemma}

\begin{proof}
	Wir verifizieren Symmetrie auf der Basis $(X_u(u,v),X_v(u,v))$ von $T(u,v)X$:
...
\begin{remark}
	Wie bei Streifen kann der Formoperator wie $\kappa_n$ durch die Gleichung 
	\[ 0=dN + \s \circ dX = dN^T + S\circ dX. \]
...
\begin{definition}
	...
	\begin{enumerate}[$\circ$]
		\item $H=\frac 12 \mathrm{tr} \s = \frac 12 \mathrm{spur}$ die \emph{mittlere Krümmung} von $X$,
...
\begin{example}
	...
	= 0 \]
	Also liefert $X_u$ und $X_v$ Krümmungsrichtungen zu Hauptrichtungen
...
\begin{remark}
	...
	\begin{enumerate}[$\circ$]
		...
		\[ \ker (id \kappa^\pm -\tilde{\s}) = A\ker (id \kappa^\pm-\s). \]
	\end{enumerate}
\end{remark}	
