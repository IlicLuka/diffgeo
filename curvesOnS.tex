...
\begin{definition} (and thm)
	$X: M \to \E^3$ is a \emph{curvature line parametrisation} if all parameter lines are curvature lines. Any surface admits locally away form umbilics, a curvature line (rep-)parametrisation.
\end{definition}
...
\begin{example}
	...
	Then
		\[ II = -2dudv. \]
	Hence the parameter line of $X$ are asymptotic lines 
...
\begin{lemma}
	Fix a point $X(u,v)$ on a parametrised surface that an asymptotic line pas through $X(u,v)$  in two, one or no directions, depending on the sign of the Gauss curvature $K(u,v)$
\end{lemma}
